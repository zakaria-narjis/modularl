...
class AbstractAgentTest(ABC):
    ...
    def test_observe(self, agent):
        ...
        agent.observe(batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones)
    ...
    def test_update(self, agent):
        ...
        assert not torch.all(torch.eq(initial_actor_state['fc.weight'], agent.actor.state_dict()['fc.weight']))

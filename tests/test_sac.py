...
def test_sac_update(sac_agent):
    ...
    assert not torch.all(torch.eq(initial_actor_state['fc.weight'], sac_agent.actor.state_dict()['fc.weight']))
    assert not torch.all(torch.eq(initial_qf1_state['fc.weight'], sac_agent.qf1.state_dict()['fc.weight']))
    assert not torch.all(torch.eq(initial_qf2_state['fc.weight'], sac_agent.qf2.state_dict()['fc.weight']))